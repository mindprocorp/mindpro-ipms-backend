...
public class CustomerResponse {
        ...
        @Builder
        @Schema(description = "고객 상세 정보 응답 객체")
        public record CustomerDetail(
                ...
                String subBizRegNo,
                @Schema(description = "사업자상호", example = "(주)마인드프로")
                String bizName,
                ...
                @Schema(description = "고객 이미지 파일 리스트")
                List<CommonRecordResponse.FileInfo> customerFileList
        ) {
        }

        @Builder
        @Schema(description = "고객 목록 응답 객체")
        ...
        @Builder
        @Schema(description = "관련 고객사 매핑 응답 객체")
        public record CustomerMappDetail(
                @Schema(description = "고객 매핑 일련번호", example = "MAP20260000001", format = "SEQ")
                String customerMappSeq,
                @Schema(description = "기준 고객 일련번호", example = "CUST20260000001")
                String customerSeq,
                @Schema(description = "연결된 업무 식별키", example = "CUST20260000005")
                String tblSeq,
                @Schema(description = "대상 고객사명", example = "상대업체명")
                String relatedCustomerName, // 추가
                @Schema(description = "고객명(영문)", example = "Gildong Hong")
                String clientNameEn,        // 추가
                @Schema(description = "출원인 주소", example = "서울특별시 강남구 테헤란로")
                String appAddress,          // 추가
                @Schema(description = "연락처 주소", example = "서울특별시 강남구 테헤란로 123")
                String contactAddress,      // 추가
                @Schema(description = "특허고객번호", example = "1-2026-123456-7")
                String kipoClientNo,        // 추가
                @Schema(description = "관계 코드", example = "REL01")
                String relationCode
        ) {
        }

        @Builder
        @Schema(description = "변경 사항 히스토리 응답 객체")
        public record ModifiedHistDetail(
                @Schema(description = "변경 이력 일련번호", example = "HIST20260000001", format = "SEQ")
                String modifiedHistSeq,
                @Schema(description = "대상 테이블 PK", example = "CUST20260000001")
                String tblSeq,
                ...
                String createUser , // SQL에서 user_name_ko를 이 필드에 매핑
        ...
        @Builder
        @Schema(description = "고객 검색(모달) 목록 응답 객체")
        public record CustomerSearchList(
                @Schema(description = "고객 일련번호", example = "CUST20260000001", format = "SEQ")
                String customerSeq,
                ...
                @Schema(description = "알수없는 항목(n_iongNo)", example = "unknown")
                String n_iongNo
        ) {}
